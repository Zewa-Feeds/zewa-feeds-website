...
const PACKS = [
  { size: "100g", price: "₹279", mrp: "₹349", sku: "C4-100G", badge: null },
  { size: "1kg", price: "₹1,999", mrp: "₹2,490", sku: "C4-1KG", badge: "Best Value" },
];

const GALLERY = [
  { type: "image", label: "Front", src: "/Bottles/Cichild/Cichild C4_Front.png" },
  { type: "video", label: "Watch", src: "/videos/brand_video.mp4", poster: "/Bottles/Cichild/Cichild C4_Front.png" },
  { type: "image", label: "Back", src: "/Bottles/Cichild/Cichild C4_back.png" },
  { type: "image", label: "Lifestyle 1", src: "/Bottles/Cichild/Cichild 01.png" },
  { type: "image", label: "Lifestyle 2", src: "/Bottles/Cichild/Cichild 02.png" },
];

const HIGHLIGHTS = [
  { title: "44% Insect Protein", sub: "Dense amino acid profile for jaw & body mass" },
  { title: "Floating Pellets", sub: "Triggers surface-strike hunting instinct" },
  { title: "100% Natural Formula", sub: "Zero synthetic additives or soy fillers" },
  { title: "High-Energy Formula", sub: "Calibrated for aggressive, active feeders" },
  { title: "1.2–1.5mm Pellet Size", sub: "Matched to cichlid mouth anatomy" },
  { title: "85% Digestibility", sub: "Clean insect protein, low waste output" },
];

const FEATURES = [
  { title: "44% Insect Protein", desc: "High-density insect meal delivers complete amino acids for the aggressive growth patterns of cichlid species." },
  { title: "Floating Pellet Design", desc: "Buoyant pellets trigger the cichlid's natural surface-strike feeding behaviour — mimicking live prey dynamics." },
  { title: "Zero Soy Fillers", desc: "No cheap plant proteins used as bulking agents — every gram is functional nutrition." },
  { title: "Jaw & Body Mass Support", desc: "Elevated protein and essential fatty acids support the strong musculature cichlids develop for territory defence." },
  { title: "Natural Colour Enhancement", desc: "Paprica and carotenoid sources intensify the vivid blues, yellows, and reds of ornamental cichlid species." },
  { title: "Balanced Mineral Profile", desc: "Calcium and phosphorus ratios support bone density and the lateral-line sensory system." },
];

const INGREDIENTS = [
  "Insect meal", "Fish meal", "Shrimp meal", "Wheat", "Corn starch",
  "Fish oil", "Insect oil", "Paprica essence", "Spirulina", "Garlic powder",
...
const NUTRITION = [
  { nutrient: "Crude Protein (min)", value: "44%" },
  { nutrient: "Crude Fat", value: "10%" },
  { nutrient: "Crude Fibre", value: "14%" },
  { nutrient: "Phosphorus", value: "2.3%" },
  { nutrient: "Calcium", value: "2.4%" },
  { nutrient: "Moisture", value: "<5%" },
  { nutrient: "Ash", value: "11%" },
];

const SUITABLE = [
  { name: "African Cichlids", desc: "Lake Malawi & Tanganyika species — colour-intensive, aggressive" },
  { name: "Oscar", desc: "Large-bodied, high-appetite carnivore" },
  { name: "Flowerhorn", desc: "Hybrid cichlid with high protein demand for hump development" },
  { name: "Convict Cichlid", desc: "Robust, territorial — thrives on high-protein diet" },
  { name: "Discus", desc: "Sensitive species benefits from clean, digestible protein" },
  { name: "Jack Dempsey", desc: "Aggressive feeder requiring dense nutrition" },
];

const RELATED = [
  { name: "Betta Bites F3", tagline: "Slow-sinking formula for bettas", href: "/products/betta-bites-f3", image: "/Bottles/Betta/Betta F3_Front.png" },
  { name: "Dried BSF Larvae 25g", tagline: "Whole larvae supplement", href: "/products/dried-bsf-larvae-25g", image: "/Bottles/DBSFL/DBSFL 25G.png" },
...
export default function CichlidBitesC4() {
      ...
      <main className="bg-[#080e1c] text-[#dde2f6] min-h-screen">
        ...
        <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16 pt-28 pb-4">
          <nav className="flex items-center gap-2 text-[12px] text-white/30 font-[Montserrat]">
            ...
            <span>/</span>
            <span className="text-white/60">Cichlid Bites C4</span>
          </nav>
        </div>
        ...
        <section className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16 py-8 lg:py-14">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            ...
            <div className="flex flex-col gap-4">
              <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-[#0d1a2e] to-[#09141e]">
              ...
              <div className="flex gap-2.5">
                {GALLERY.map((item, i) => (
                  <button key={i} onClick={() => setActiveImage(i)}
                    className={`relative flex-1 aspect-square rounded-xl border overflow-hidden transition-all duration-200 ${activeImage === i ? "shadow-lg" : "border-white/8 hover:border-white/25"}`}
            ...
            <div className="flex flex-col gap-6 lg:pt-2">
              ...
              <div>
                ...
                <p className="text-[15px] text-white/55 leading-relaxed font-[Montserrat] max-w-md">
                  High-energy floating pellets for aggressive cichlid species — 44% insect protein, zero soy fillers.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[12px] text-white/35 font-[Montserrat] ml-1">4.8 · 94 reviews</span>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button onClick={handleAddToCart}
                    className={`flex-1 py-4 px-6 font-bold text-[13px] tracking-widest uppercase font-[Montserrat] active:scale-[0.98] transition-all rounded-sm flex items-center justify-center gap-2 ${added ? "bg-emerald-500 text-white" : "text-[#00382d] hover:opacity-90"}`}
                    style={{ background: added ? undefined : accent }}>
                  ...
                  <button onClick={handleBuyNow}
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = accent; e.currentTarget.style.color = "#00382d"; }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-4xl">
              {FEATURES.map((f) => (
                <div key={f.title} className="flex flex-col gap-2">
                  <h4 className="text-[14px] font-semibold text-white font-[Montserrat]" style={{ color: accent }}>{f.title}</h4>
