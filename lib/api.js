...
/** Server-side/ISR fetch for cacheable public data with timeout & hibernation wake-up retry. */
async function cached(path, revalidate = 60, retries = 1) {
  ...
  try {
    ...
    if (response.status === 503 && retries > 0) {
      ...
      return cached(path, revalidate, retries - 1);
    }
